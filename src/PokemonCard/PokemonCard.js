...
class PokemonCard extends React.Component {
  constructor () {
    ...
    this.priorPokemon = this.priorPokemon.bind(this)
  ...
  handleClick () {
    ...
    this.setPokemonTo(rand)
  }

  nextPokemon() {
   const current = this.state.number;
   this.setPokemonTo(current + 1);
 }
 ...
 priorPokemon() {
  ...
  render() {
    ...
    return (
      <div style={ genStyle }>
      <Card className={classes.card} raised="true" style={{backgroundColor: this.state.lightColor}}>
        <CardActionArea>
        <CardMedia
          component="img"
          alt="Pokemon"
        ...
        />
        <CardContent>
          <Typography gutterBottom variant="h5" component="h2" style={{color:this.state.darkColor}}>
            {this.capitalizeFirstLetter(this.state.name)}
          </Typography>
          <Typography component="p" style={{color:this.state.darkColor}}>
            This is {this.capitalizeFirstLetter(this.state.name)}. It weighs {this.state.weight}kg  and is {this.state.height} inches tall.
          </Typography>
        </CardContent>
        </CardActionArea>
        <CardActions>
        // TODO add animations
        // TODO - add next/prior button, find by number, find by nam
        <Button size="small" color="primary" onClick={this.priorPokemon}>
          Previous
        </Button>
        <Button size="small" color="primary" onClick={this.handleClick}>
          Random
        </Button>
        <Button size="small" color="primary" onClick={this.nextPokemon}>
          Next
        </Button>
        </CardActions>
      </Card>
